...
class TestFileMethods(unittest.TestCase):
    ...
    def test_flat(self):
        actual = text_parser.flat([(1, 2), (3, 4)])
        expected = [1, 2, 3, 4]
        self.assertEqual(actual, expected)

    def test_is_verb(self):
        self.assertTrue(text_parser.is_verb("run"))
        self.assertFalse(text_parser.is_verb("wall"))

    def test_get_trees(self):
        path = r"./django/tests/model_forms"
        actual = [str(i) for i in text_parser.get_trees(path)]
    ...
    def test_get_top_function_names_in_path(self):
        ...
        actual = text_parser.get_top_functions_names_in_path(path)
